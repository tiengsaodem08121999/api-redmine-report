...
class RedmineService
{
    ...
    public function verifyRedmineLogtimeAgainstReport()
    {
        $logtimeHistoryGroupedByDate = HistoryDailyLogtime::whereMonth('date', Carbon::now()->month)
            ->whereYear('date', Carbon::now()->year)->get()->toArray();
        
        $logtimeRedmine = $this->getLogtimeForThisMonth();
        $errors = [];

        $historyMap = [];
        foreach ($logtimeHistoryGroupedByDate as $item) {
            $date = $item['date'];
            $member = $item['member'];
            $historyMap[$date][$member][] = $item;
        }
    
        $redmineMap = [];
        foreach ($logtimeRedmine as $date => $logs) {
            foreach ($logs as $log) {
                $member = $log['user']['name'];
                $redmineMap[$date][$member][] = $log;
            }
        }
    
        $allDates = array_unique(array_merge(array_keys($historyMap), array_keys($redmineMap)));
    
        foreach ($allDates as $date) {
            $historyMembers = isset($historyMap[$date]) ? array_keys($historyMap[$date]) : [];
            $redmineMembers = isset($redmineMap[$date]) ? array_keys($redmineMap[$date]) : [];
            $allMembers = array_unique(array_merge($historyMembers, $redmineMembers));
    
            foreach ($allMembers as $member) {
                $historyLogs = $historyMap[$date][$member] ?? [];
                $redmineLogs = $redmineMap[$date][$member] ?? [];
    
                if (empty($historyLogs)) {
                    $errors[$date][$member][] = "report thiếu task";
                    continue;
                }
    
                if (empty($redmineLogs)) {
                    $errors[$date][$member][] = "log time thiếu";
                    continue;
                }
    
                foreach ($historyLogs as $hLog) {
                    $matched = false;
                    foreach ($redmineLogs as $rLog) {
                        $sameTask = intval($hLog['task_id']) == $rLog['issue']['id'];
                        $sameMember = $hLog['member'] == $rLog['user']['name'];
                        $sameDate = $hLog['date'] == $rLog['spent_on'];
                        $sameTime = floatval($hLog['spent_time']) == floatval($rLog['hours']);
    
                        if ($sameMember && $sameDate) {
                            if (!$sameTask) {
                                $errors[$date][$member][] = "log time sai report";
                                $matched = true;
                                break;
                            }
                            if ($sameTask && !$sameTime) {
                                $errors[$date][$member][] = "log time sai spenttime";
                                $matched = true;
                                break;
                            }
                            if ($sameTask && $sameTime) {
                    ...
                    if (!$matched) {
                        $errors[$date][$member][] = "$member log time không khớp chi tiết nào";
                    }
                }
            }
        }
